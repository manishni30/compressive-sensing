...
for i=1:1:n
    ...
    %sensor data
    S(i).data = randn(1)*2 + 45;
    
    %neighbour sensor data
    S(i).neigh_data = zeros(15,1);
    
    %counter for storing neighbour data
    S(i).count = 1;
...
for i = 1:n
    ...
    if S(pt).count < 15
        S(pt).neigh_data(S(pt).count) = S(i).data;
        S(pt).count = S(pt).count + 1;
    end
    
end

%cluster head send data to the sink
upto_fill = 0;
s = 3;
for i = 0:length(C)-1
    ...
    sink.data(upto_fill+1: upto_fill + M,1) = y;
    
    sink.data(upto_fill+M+1:upto_fill+N,1) = 0;
    
    sink.data(upto_fill+N+1,1) = -1;
    sink.data(upto_fill+N+1,2) = M;
    
    sink.data(upto_fill+1:upto_fill+N,2) = clust_point;
    
    upto_fill = upto_fill + N + 1;
end

stoping_point = find(sink.data(:,1) == -1);

start = 1;
for j = 1:length(stoping_point)

k = stoping_point(j);
data = sink.data(start:k,:);
M = sink.data(k,2); 
y = sink.data(start:start+M-1,1);
clust_point = sink.data(k-1,2);

re_x = reconstruct(y,length(data)-1,clust_point);

sink.data(start:k-1,3) = re_x; 
start = k+1;
end

figure()

cluster_head = sink.data(stoping_point-1,2);

actual_data = zeros(200,1);
reconstruct_data = zeros(200,1);
start = 1;
start_s = 1;
for j = 1:length(cluster_head)
    data = S(cluster_head(j,1)).neigh_data;
    actual_data(start:start+length(data)-2,:) = data(1:length(data)-1,:);
    
    data = sink.data(start_s:stoping_point(j,1)-1,3);
    reconstruct_data(start:start+length(data)-2,:) = data(1:length(data)-1,:);
    start = start+ length(data);
    start_s = stoping_point(j,1)+1;
end

plot(actual_data,'r*');
hold on;
plot(reconstruct_data,'b*');
end
% figure1 = figure11;
% % Create axes
